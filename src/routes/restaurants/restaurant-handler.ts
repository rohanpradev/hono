...
import type { GetRestaurant, GetRestaurantById } from "@/routes/restaurants/restaurant.routes";
...
export const getRestaurantById: AppRoutehandler<GetRestaurantById> = async (c) => {
  const id = c.req.valid("param").id;
  const restaurantsData = await RestaurantService.getRestaurantById(id);
  if (!restaurantsData)
    return c.json({ message: `Data ${HttpStatusPhrases.NOT_FOUND}` }, HttpStatusCodes.NOT_FOUND);
  const { city, ...rest } = restaurantsData;
  return c.json({ ...rest, city: city.name, state: city.state.name, code: city.state.code }, HttpStatusCodes.OK);
};
